...
export const getContent = (films) => {
  ...
  return `${menuPanel(getFilteredFilmsCount(films, `isWatchedList`), getFilteredFilmsCount(films, `isHistory`), getFilteredFilmsCount(films, `isFavorite`))}
  ${sortingPanel()}
  ${filmsWrapper(
    // getFilmCardsFrom(films, 0, FILMS_SIZE_FOR_RENDER_AT_LINE), createFilmCard(sortedTopRatedFilms[0]) + createFilmCard(sortedTopRatedFilms[1]), createFilmCard(sortedCommentedFilms[0]) + createFilmCard(sortedCommentedFilms[1])
  )}`;
};
